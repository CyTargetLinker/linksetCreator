...
import org.linkset.creator.utils.ArgsParser;
import org.linkset.creator.utils.ConfigAttributes;
import org.linkset.creator.utils.ConfigFileReader;
import org.linkset.creator.utils.Utils;
import org.linkset.creator.utils.ArgsParser.AFilesIn;
import org.linkset.creator.utils.ArgsParser.AFilesOut;
import org.linkset.creator.utils.ArgsParser.AHelp;
import org.linkset.creator.utils.ArgsParser.GraphBuilder;
...
public class GenericCreator {
	...
	public void startConversion(Args pargs) throws Exception {
		if(pargs.isInput() && pargs.isConfig() && pargs.isOutput()) {
			...
			if(in.exists() && config.exists()) {				
				...
				System.out.println("cannot find input and/or config file");
			}
		} else {
	...
	private String getIdentifiers(String identifier, String targetSyscodeIn,
		...
		ids.add(identifier);
		Xref in = new Xref(identifier, DataSource.getExistingBySystemCode(targetSyscodeIn));
		for(String ds : targetSyscodeOut) {
			Set<Xref> result = mapper.mapID(in, DataSource.getExistingBySystemCode(ds));
			for(Xref x : result) {
				ids.add(x.getId());
			}
		}
	...
	private void registerNode(Node node, String syscodeIn, IDMapper mapper) throws IDMapperException {
		...
		Xref xref = new Xref(id, DataSource.getExistingBySystemCode(syscodeIn));
		if(mapper != null) {
			// if node is a microRNA check if there is a MIMAT identifier
			if(syscodeIn.equals("Mb")) {
				Set<Xref> result = mapper.mapID(xref, DataSource.getExistingBySystemCode("Mbm"));
				boolean found = false;
				for(Xref x : result) {
			...
			Set<Xref> result = mapper.mapID(xref, DataSource.getExistingBySystemCode(syscodeIn));
			for(Xref x : result) {
